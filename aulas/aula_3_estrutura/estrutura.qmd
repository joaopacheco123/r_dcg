...
# Nós podemos alterar "idade" para a classe de número inteiro usando a função as.integer()
dados_turma_1$idade <- as.integer(dados_turma_1$idade)

# Veja que mudou:
class(dados_turma_1$idade)

# Agora a nossa função irá reconhecer que temos uma variável de número inteiro no banco de dados.
colunas_com_inteiro <- colnames(dados_turma_1)[sapply(dados_turma_1, is.integer)]
```
...
# Usando a função table, conseguimos fazer uma tabulação cruzada das variáveis música e cinema.
